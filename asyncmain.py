...
def main(argv):
    try:
        opts, args = getopt.getopt(argv,"s:x:h:c:p:k:e:r:",["server=","port=","ca_certs=","certfile=", "keyfile=", "execute_topic=","response_topic="])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', '--execute_topic', required=False, default="raspberry/commands", help="By defaukt commands are received in topic: raspberry/commands")
    parser.add_argument('-r', '--response_topic', required=False, default="raspberry/state", help="By defaukt state are published in topic raspberry/commands")
    ...
    parser.add_argument('-d', '--disable-clean-session', default=False, action='store_true', help="disable 'clean session' (sub + msgs not cleared when client disconnects)")
    ...
    if usetls:
        ...
        if not args.insecure:
            ...
            keyfile = None
        print tlsVersion
        mqttc.tls_set(ca_certs=args.cacerts, certfile=certfile, keyfile=keyfile, cert_reqs=cert_required, tls_version=tlsVersion)
